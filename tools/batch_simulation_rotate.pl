...
sub launch_jobs {
  ...
  for(my $i = 0; $i < $opt_j; $i++) {
    ...
    $cmd .=   "--DescriptorDir /<install dir>/cogs/metrics/faked2 --SocksPort ".(9000 + $i)." ";
    ...
    $cmd .=   "--NumEntryGuards $opt_n --MaliciousIntroDate $opt_m --MaliciousBandwidth $MBW > out.$i.$num_evil.txt";
