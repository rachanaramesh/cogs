...
int onion_extend_cpath(origin_circuit_t *circ);
...
get_consensus_time()
{
    //return (current_consensus ? current_consensus->valid_after : 0);
    tor_assert(get_options()->Time || get_options()->StartConsensus);

    if (!get_options()->Time) {
        get_options()->Time = get_options()->StartConsensus;
    }

    return get_options()->Time;
}
...
onion_populate_cpath(origin_circuit_t *circ)
{
 ...
 again:
  ...
  if (r < 0) {
        // Kevin
        log_notice(LD_CIRC,"Generating cpath hop failed.");
    return -1;
  }
  if (r == 0)
    goto again;
...
choose_good_entry_server(uint8_t purpose, cpath_build_state_t *state)
{
  ...
  if (state && options->UseEntryGuards &&
    ...
    printf("choose_good_entry_server calls choose_random_entry\n");
...
onion_extend_cpath(origin_circuit_t *circ)
{
  ...
  if (!info) {
    ...
    // Kevin
    log_info(LD_CIRC,"Chose router %s for hop %d (exit is %s)",
...
entry_is_time_to_retry(entry_guard_t *e, time_t now)
{
  ...
  diff = now - e->unreachable_since;
    if (diff < 6*60*60) {
        printf("It's been less than 6 hours since %s was down. Returning %d\n", e->nickname, now > (e->last_attempted + 60*60));
    return now > (e->last_attempted + 60*60);
    }
    else if (diff < 3*24*60*60) {
        printf("It's been 6 hours since %s was down. Returning %d\n", e->nickname, now > (e->last_attempted + 4*60*60));
    return now > (e->last_attempted + 4*60*60);
    }
    else if (diff < 7*24*60*60) {
        printf("It's been 3 days since %s was down. Returning %d\n", e->nickname, now > (e->last_attempted + 18*60*60));
    return now > (e->last_attempted + 18*60*60);
    }
    else {
        printf("It's been a week since %s was down. Redutning %d\n", e->nickname, now > (e->last_attempted + 36*60*60));
              ...
              int assume_reachable, const char **msg)
{
  ...
  /* no good if it's unreachable, unless assume_unreachable or can_retry. */
    if (get_options()->Simulate) {
        the_time = get_consensus_time();
    } else {
        the_time = time(NULL);
    }
  if (!assume_reachable && !e->can_retry &&
            e->unreachable_since && !entry_is_time_to_retry(e, the_time)) {
    *msg = "unreachable";
    return NULL;
  }
  ...
  r = router_get_by_digest(e->identity);
    ...
    if (get_options()->Simulate) {
        if (!r) {
            entry_guard_register_connect_status(e->identity, 0,
                                                0, get_consensus_time());
        } else {
            entry_guard_register_connect_status(e->identity, 1,
                                                0, get_consensus_time());
        }
        return r;
    }
...
log_entry_guards(int severity)
{
  ...
  char *s;
    ...
    // Kevin
    if (smartlist_len(entry_guards) < get_options()->NumEntryGuards) {
        smartlist_free(elements);
        return;
    }
...
add_an_entry_guard(routerinfo_t *chosen, int reset_status)
{
  ...
  if (chosen) {
    ...
    if (!router) {
      printf("choose_good_entry_server() returned NULL\n");
      exit(0);
      return NULL;
    }

    // Kevin
    //printf("add_an_entry_guard(): chose %s (%d)\n", router->nickname, router->is_possible_guard);
  }
  entry = tor_malloc_zero(sizeof(entry_guard_t));
  //  printf("Chose '%s' as new entry guard.\n",
  ...
  if (get_options()->Simulate) {
    entry->chosen_on_date = get_consensus_time() - crypto_rand_int(3600*24*get_options()->GuardRotateMinDays);
  } else {
...
pick_entry_guards(or_options_t *options)
{
  ...
  //  printf("Number of live entry guards: %d\n", num_live_entry_guards());
  //  printf("Should be patient? %d\n", should_be_patient());
  
  //while((num_live_entry_guards() < options->NumEntryGuards &&
  //	 !should_be_patient()) || num_live_entry_guards() == 0) {  
  
  while(num_live_entry_guards() < options->NumEntryGuards)
    {
    // Kevin
    //printf("num_live_entry_guards() < options->NumEntryGuards\n");
...
remove_obsolete_entry_guards(time_t now)
{
  int changed = 0, i;
    ...
    log_info(LD_OR, "In remove_obsolete_entry_guards(): num guards %d",
  ...
  for (i = 0; i < smartlist_len(entry_guards); ++i) {
    ...
    if (!ver) {
      ...
      tor_free(tor_ver);
        ...
        if ((!get_options()->Simulate && !version_is_bad && entry->chosen_on_date + 3600*24*60 < now) ||
                (get_options()->Simulate && !version_is_bad && !get_options()->NoChurn &&
                 (entry->chosen_on_date + 3600*24*get_options()->GuardRotateMaxDays < get_consensus_time()))) {
      /* It's been 2 months since the date listed in our state file. */
      msg = "was selected several months ago";
      date_is_bad = 1;
            printf("Removing guard node %s that was too old \n", entry->nickname);
    }
    ...
    if (version_is_bad || date_is_bad) { /* we need to drop it */
      ...
      base16_encode(dbuf, sizeof(dbuf), entry->identity, DIGEST_LEN);
            ...
            //printf("Entry guard '%s' (%s) %s. (Version=%s.) Replacing it.",
  ...
  SMARTLIST_FOREACH_BEGIN(entry_guards, entry_guard_t *, entry)
    {
      ...
      const char *reason = NULL;
        ...
        changed = 1;
            //printf("Guard down: %s Reason: %s \n", entry->nickname, reason);
        }
                                    ...
                                    int mark_relay_status, time_t now)
{
  ...
  SMARTLIST_FOREACH(entry_guards, entry_guard_t *, e,
    {
        //log_notice(LD_OR, "Router %s unreachable_since: %d last_attempted: %d and success? %d", e->nickname, e->unreachable_since, e->last_attempted, succeeded);
  ...
  if (succeeded) {
    if (entry->unreachable_since) {
            ...
            //printf("Entry guard '%s' (%s) is now reachable again. Good. (Was down since %s)\n",
    ...
    if (!entry->made_contact) {
      ...
      changed = 1;
            ...
            // Kevin
            //printf("Unable to connect to entry guard '%s' (%s). "
      ...
      format_iso_time(tbuf, entry->unreachable_since);
            // Kevin
            //printf("Failed to connect to unreachable entry guard "
...
should_be_patient()
{
  SMARTLIST_FOREACH_BEGIN(entry_guards, entry_guard_t *, entry)
    {
      //      printf("should_be_patient(): router: %s unreachable since: %d, consensus time: %d, patience time: %d", entry->nickname, entry->unreachable_since, get_consensus_time(), 3600*get_options()->Patience);
      if(entry->unreachable_since > 0 && 
         (get_consensus_time() - entry->unreachable_since) < (3600*get_options()->Patience))
	return 1;
    }
  SMARTLIST_FOREACH_END(entry);
  return 0;
...
choose_random_entry(cpath_build_state_t *state)
{
 ...
 retry:
  ...
  //  if (entry_list_is_constrained(options) || options->NumEntryGuards == 1) {
  if (entry_list_is_constrained(options)) {
  ...
  //Tariq testing the live versus preffered min guard test.
  //printf("Live guards = %d, Preferred Min = %d\n", smartlist_len(live_entry_guards), preferred_min);
 ...
 choose_and_finish:
  if (entry_list_is_constrained(options)) {
     ...
     * *double*-weight our guard selection. */
        ...
        // Kevin
        /*
        SMARTLIST_FOREACH(live_entry_guards, routerinfo_t *, e,
        {
            char dbuf[HEX_DIGEST_LEN+1];
            base16_encode(dbuf, sizeof(dbuf), e->cache_info.identity_digest, DIGEST_LEN);
  ...
  *msg = NULL;
        ...
        *msg = tor_strdup("Unable to parse entry nodes: "
        ...
        if (base16_decode(node->identity, DIGEST_LEN, smartlist_get(args,1),
          ...
          *msg = tor_strdup("Unable to parse entry nodes: "
            ...
            /*
            char dbuf[HEX_DIGEST_LEN+1];
            base16_encode(dbuf, sizeof(dbuf), node->identity, DIGEST_LEN);
            dbuf[HEX_DIGEST_LEN] = '\0';
            printf("%s %s\n", node->nickname, dbuf);
            */
      SMARTLIST_FOREACH(args, char*, cp, tor_free(cp));
       ...
       *sp++ = '\0';
       ...
       if (state_version) {
         e->chosen_by_version = tor_strdup(state_version);
                // Kevin (probably a no-op)
                if (get_options()->Simulate) {
                    e->chosen_on_date = get_consensus_time() - crypto_rand_int(3600*24*get_options()->GuardRotateMinDays);
                } else {
  ...
  if (*msg || !set) {
    ...
    smartlist_free(new_entry_guards);
        ...
        log_info(LD_OR, "entry_guards_parse_state(): No errors in parsing state file");
    ...
    entry_guards = new_entry_guards;
        log_info(LD_OR, "Entry guard list size: %d new list %d", smartlist_len(entry_guards),
                 smartlist_len(new_entry_guards));
  ...
  digestmap_free(added_by, _tor_free);
    ...
    // log_notice(LD_OR, "Returning msg: %s set: %d", *msg, set);
  ...
  SMARTLIST_FOREACH(entry_guards, entry_guard_t *, e,
    {
      char dbuf[HEX_DIGEST_LEN+1];
        ...
        dbuf[HEX_DIGEST_LEN] = '\0';
        printf("%s %s %s\n", e->nickname, dbuf, e->unreachable_since ? "down" : "up");
        */
