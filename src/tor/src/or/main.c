...
#define DEFAULT_MALICIOUS_BW (1024 * 1024 * 10)
#define DEFAULT_MALICIOUS_BW_KBPS (DEFAULT_MALICIOUS_BW / 1024)
#define MALICIOUS_NICK_BASE "evilguy"
...
static int simulate_node_selection(void);

// Kevin
void remove_all_routers(void);
routerinfo_t *build_malicious_routerinfo_t(int);
vote_routerstatus_t *build_malicious_routerstatus_t(int id);
void insert_malicious_nodes(void);
int setup_and_load_next_simulation_files(void);
void choose_circuit(void);
void print_routerlist(void);
void build_exit_list(void);

/********* START VARIABLES **********/

// Kevin
extern networkstatus_t *current_consensus;
static smartlist_t *client_list;
static smartlist_t *exit_node_list = NULL;
extern double *bandwidths;
extern uint32_t *ubandwidths;
extern bitarray_t *exit_bits;
extern bitarray_t *guard_bits;
...
print_routerlist()
{
    ...
    if (ri->is_running && ri->is_valid && ri->is_possible_guard) {
    char dbuf[HEX_DIGEST_LEN+1];
        base16_encode(dbuf, sizeof(dbuf), ri->cache_info.identity_digest, DIGEST_LEN);
        dbuf[HEX_DIGEST_LEN] = '\0';
	//        printf("list_contains %s %s bw: %d\n", ri->nickname, dbuf, ri->bandwidthrate);
    }
                     );
}

void
update_bandwidth_weights()
{
    double T=0, D=0, E=0, G=0, M=0;
    uint32_t weight_scale = BW_WEIGHT_SCALE;
    char *buf = NULL, *item = NULL, *ptr = NULL;
    smartlist_t *chunks = NULL;

    tor_assert(get_options()->Simulate);
    tor_assert(current_consensus);

    if (current_consensus->consensus_method < MIN_METHOD_FOR_BW_WEIGHTS)
        return;

    SMARTLIST_FOREACH(current_consensus->routerstatus_list, vote_routerstatus_t *, rs,
    if (rs->status.has_bandwidth) {
    T += rs->status.bandwidth;
    if (rs->status.is_exit && rs->status.is_possible_guard)
            D += rs->status.bandwidth;
        else if (rs->status.is_exit)
            E += rs->status.bandwidth;
        else if (rs->status.is_possible_guard)
            G += rs->status.bandwidth;
        else
            M += rs->status.bandwidth;
    }
                     );

    chunks = smartlist_create();

    if (!current_consensus->weight_params) {
        printf("WARN: current_consensus->weight_params is NULL, ");
        printf("yet consensus_method >= %d\n", MIN_METHOD_FOR_BW_WEIGHTS);
        return;
    }
/*
    SMARTLIST_FOREACH(current_consensus->weight_params, char *, buf,
                      printf("Original Weights: %s\n", buf);
                     );
*/
    SMARTLIST_FOREACH(current_consensus->weight_params, char *, c, tor_free(c));
    smartlist_free(current_consensus->weight_params);
    current_consensus->weight_params = smartlist_create();

    if (current_consensus->consensus_method < 10) {
        networkstatus_compute_bw_weights_v9(chunks, G,
                                            M, E, D,
                                            T, weight_scale);
        buf = smartlist_get(chunks, 1);
    } else {
        networkstatus_compute_bw_weights_v10(chunks, G,
                                             M, E, D,
                                             T, weight_scale);
        buf = smartlist_get(chunks, 0);
    }

    item = strtok(buf, " ");

    while (item) {
        if ( strcmp("bandwidth-weights",item) != 0) {
            ptr = tor_malloc(strlen(item) + 1);
            strncpy(ptr, item, strlen(item) + 1);
            smartlist_add(current_consensus->weight_params, ptr);
        }
        item = strtok(NULL, " ");
    }

    /*SMARTLIST_FOREACH(current_consensus->weight_params, char *, buf,
                      printf("New Weights: %s\n", buf);
                     );*/
...
// Kevin
vote_routerstatus_t *
build_malicious_routerstatus_t(int id)
{
    vote_routerstatus_t *vrs;
    vrs = tor_malloc_zero(sizeof(vote_routerstatus_t));

    routerstatus_t *rs = &vrs->status;

    // Set nick name
    snprintf(rs->nickname, MAX_NICKNAME_LEN,
             "%s%d", MALICIOUS_NICK_BASE, id);

    // Set identity digest to our id (to ensure uniqueness)
    memset(&rs->identity_digest, 0, sizeof(rs->identity_digest));
    memcpy(&rs->identity_digest, (void *) &id, sizeof(int));

    // Set descriptor digest to our id (to ensure uniqueness)
    memset(&rs->descriptor_digest, 0, sizeof(rs->descriptor_digest));
    memcpy(&rs->descriptor_digest, (void *) &id, sizeof(int));

    // Give us an arbitrary or and dir port
    rs->or_port = 9001;
    rs->dir_port = 9002;

    // Set published on date
    rs->published_on = get_consensus_time();
    ...
    rs->is_fast = 1;
    rs->is_stable =1;
    rs->is_possible_guard = get_options()->MaliciousGuard;
    rs->is_exit = get_options()->MaliciousExit;
    rs->has_bandwidth = 1;
    rs->has_measured_bw = 1;
    rs->evil = 1;

    // set bandwidth
    rs->bandwidth = get_options()->MaliciousBandwidth ? get_options()->MaliciousBandwidth / 1024.0 : DEFAULT_MALICIOUS_BW_KBPS;
    rs->measured_bw = get_options()->MaliciousBandwidth ? get_options()->MaliciousBandwidth / 1024.0 : DEFAULT_MALICIOUS_BW_KBPS;

    // TODO: Need to set flags and consensus bandwidth
...
// Kevin
routerinfo_t *
build_malicious_routerinfo_t(int id)
{
    routerinfo_t *ri;
    ...
    // Set nick name
    ri->nickname = tor_malloc_zero(MAX_NICKNAME_LEN+1);
    snprintf(ri->nickname, MAX_NICKNAME_LEN,
             "%s%d", MALICIOUS_NICK_BASE, id);

    // Set identity digest to our id (to ensure uniqueness)
    memset(&ri->cache_info.identity_digest, 0, sizeof(ri->cache_info.identity_digest));
    memcpy(&ri->cache_info.identity_digest, (void *) &id, sizeof(int));

    // Set descriptor digest to our id (to ensure uniqueness)
    memset(&ri->cache_info.signed_descriptor_digest, 0, sizeof(ri->cache_info.signed_descriptor_digest));
    memcpy(&ri->cache_info.signed_descriptor_digest, (void *) &id, sizeof(int));
    ...
    // Set published on date
    ri->cache_info.published_on = get_consensus_time();

    // set empty platform
    ri->platform = tor_malloc_zero(32);

    // set self-reported bandwidths
    ri->bandwidthrate = get_options()->MaliciousBandwidth ? get_options()->MaliciousBandwidth : DEFAULT_MALICIOUS_BW;
    ri->bandwidthburst = get_options()->MaliciousBandwidth ? get_options()->MaliciousBandwidth : DEFAULT_MALICIOUS_BW;
    ri->bandwidthcapacity = get_options()->MaliciousBandwidth ? get_options()->MaliciousBandwidth : DEFAULT_MALICIOUS_BW;
    ...
    ri->is_fast = 1;
    ri->is_stable =1;
    ri->is_possible_guard = get_options()->MaliciousGuard;
    ri->is_exit = get_options()->MaliciousExit;
    ri->evil = 1;
...
// Kevin
void
insert_malicious_nodes()
{
    uint32_t i;
    if (get_options()->NumMaliciousNodes > 0) {
        routerlist_t *rl = router_get_routerlist();
        for (i = 0; i < get_options()->NumMaliciousNodes; i++) {
            // Insert into routerinfo_t list
            routerinfo_t *ri = build_malicious_routerinfo_t(i+1);
            routerlist_insert(rl, ri);

            // Insert into routerstatus_t list
            vote_routerstatus_t *vrs = build_malicious_routerstatus_t(i+1);
            smartlist_add(current_consensus->routerstatus_list, vrs);
        }

        // Be sure that our lists are still sorted by identity fingerprint
        routers_sort_by_identity(rl->routers);
        i = 0;

        // Reset the index after sorting
        SMARTLIST_FOREACH(rl->routers, routerinfo_t *, ri,
                          ri->cache_info.routerlist_index = i;
                          i++;
                         );

        routerstatus_sort_by_identity(current_consensus->routerstatus_list);
    }
}
...
void remove_all_routers()
{
    ...
    if (routerlist) {
        SMARTLIST_FOREACH(routerlist->routers, routerinfo_t *, ri,
                          routerlist_remove(routerlist, ri, 0, get_consensus_time());
                         );
    }

    if (current_consensus) {
        SMARTLIST_FOREACH(current_consensus->routerstatus_list, vote_routerstatus_t *, rs,
                          smartlist_remove(current_consensus->routerstatus_list, rs);
                         );
    }
}
...
// Kevin
int
setup_and_load_next_simulation_files()
{
    char confilename[FILENAME_MAX];
    char desfilename[FILENAME_MAX];
    char filetime[FILENAME_MAX];
    struct tm *ts;
    time_t time = get_options()->Time;
    struct stat filestat;

    // Remove any currently loaded routers
    remove_all_routers();

    ts = gmtime((const time_t *) &time);

    log_info(LD_OR, "Year:%d Month:%d Day:%d Hour:%d Min:%d Sec:%d",
             ts->tm_year+1900, ts->tm_mon+1, ts->tm_mday, ts->tm_hour, ts->tm_min, ts->tm_sec);

    strftime(filetime, sizeof(filetime), "%Y-%m-%d-%H-%M-%S", ts);

    log_info(LD_OR, "strftime is %s", filetime);

    snprintf(confilename, sizeof(confilename), "%s/%s-consensus", get_options()->ConsensusDir, filetime);

    if (get_options()->ConsensusFile) {
        tor_free(get_options()->ConsensusFile);
        get_options()->ConsensusFile = tor_malloc_zero(FILENAME_MAX);
    }

    strncpy(get_options()->ConsensusFile, confilename, FILENAME_MAX);

    snprintf(desfilename, sizeof(desfilename), "%s/%s-descriptors", get_options()->DescriptorDir, filetime);

    if (get_options()->DescriptorFile) {
        tor_free(get_options()->DescriptorFile);
        get_options()->DescriptorFile = tor_malloc_zero(FILENAME_MAX);
    }

    strncpy(get_options()->DescriptorFile, desfilename, FILENAME_MAX);

    log_info(LD_OR, "cfilename:%s dfilename:%s Time:%d", confilename, desfilename, get_options()->Time);

    if (stat(get_options()->DescriptorFile, &filestat) < 0) {
      //log_notice(LD_OR, "Cannot stat descriptors file %s", get_options()->DescriptorFile);
        return -1;
    }

    if (stat(get_options()->ConsensusFile, &filestat) < 0) {
      //log_notice(LD_OR, "Cannot stat consensus file %s", get_options()->ConsensusFile);
        return -1;
    }
...
do_main_loop(void)
{
  ...
  if (router_reload_router_list()) {
    return -1;
        ...
        printf("Summary:\nClients: %d\nPaths: %d\nStart: %d\nStop: %d\nTotalHours: %d\n"
               "DescriptorDir: %s\nConsensusDir: %s\nNumMalicious: %d\nTotalMaliciousBw: %d\n"
               "GuardRotateMinDays: %d days\nGuardRotateMaxDays: %d days\n"
               "MaliciousGuard: %d\nMaliciousExit: %d\n"
	       "Patience: %d\nGuard Rotate? (0=Y, 1=N) %d\nEntry Guards: %d\n",
               get_options()->NumClients,
               get_options()->NumPaths,
               get_options()->StartConsensus,
               get_options()->StopConsensus,
               1 + ((get_options()->StopConsensus - get_options()->StartConsensus) / ONE_HOUR),
               get_options()->DescriptorDir,
               get_options()->ConsensusDir,
               get_options()->NumMaliciousNodes,
               (get_options()->NumMaliciousNodes *
                (get_options()->MaliciousBandwidth ?
                 get_options()->MaliciousBandwidth : DEFAULT_MALICIOUS_BW)),
               get_options()->GuardRotateMinDays, get_options()->GuardRotateMaxDays,
               get_options()->MaliciousGuard, get_options()->MaliciousExit, (get_options()->Patience ? get_options()->Patience : 0), get_options()->NoChurn, get_options()->NumEntryGuards);

        // Initialize client list
        client_list = smartlist_create();
        for (i = 0; i < get_options()->NumClients; i++) {
            or_state_t *client_state = init_client_state();
            client_state->id = i; // set client id
            smartlist_add(client_list, (void *) client_state);
        }

        // Iterate over each consensus we want to simulate
        for (get_options()->Time = get_options()->StartConsensus;
                get_options()->Time <= get_options()->StopConsensus;
                get_options()->Time += ONE_HOUR) {

            // Setup next router list
            if (setup_and_load_next_simulation_files() < 0) {
                log_notice(LD_OR, "Simulation file loading failed. Skipping.");
                continue;
            }

            // Insert malicious nodes
            if (get_options()->MaliciousIntroDate <= get_options()->Time) {
                insert_malicious_nodes();
                update_bandwidth_weights();
                //printf("Time %d had malicious guards\n", get_options()->Time);
            }
            ...
            routerlist_t *rl = router_get_routerlist();
            printf("Using consensus file %s and descriptor file %s (time %d)\n",
                   get_options()->ConsensusFile == NULL ?
                   "<default>" : get_options()->ConsensusFile,
                   get_options()->DescriptorFile == NULL ?
                   "<default>" : get_options()->DescriptorFile,
                   get_options()->Time);
            printf("Starting path selection simulation with %d routers\n",
                   smartlist_len(rl->routers));

            // Build smartlist of routerstatuses that exit to port 80
            build_exit_list();

            // Simulate path selection for NumClients clients
            for (i = 0; i < get_options()->NumClients; i++) {
                or_state_t *client_state = smartlist_get(client_list, i);

                printf("Client %d\n", client_state->id + 1);

                or_state_t *global_state = get_or_state();
                global_state = client_state;

                if (entry_guards_parse_state(global_state, 1, &err) < 0) {
                    printf("Error in entry_guards_parse_state(): %s\n", err);
                }

		if (simulate_node_selection() < 0) {
                    log_notice(LD_OR, "Path simulation failed. Skipping.");
                } else {
                    entry_guards_update_state(global_state);
		    log_entry_guards(LOG_SIM);

                    for (j = 0; j < get_options()->NumPaths; j++) {
                        choose_circuit();
                    }
                }
            }
...
// Kevin
/*
  Simulate node selection according to Tor's default algorithms.
  Returns 0 on success, -1 on failure.
*/

static int
simulate_node_selection(void)
{
    routerlist_t *rl = NULL;

    tor_assert(get_options()->Simulate);

    rl = router_get_routerlist();

    if (!(rl && smartlist_len(rl->routers))) {
        log_notice(LD_OR, "Warn: No router descriptors have been loaded. Are we missing %s and/or %s?",
                   get_options()->ConsensusFile, get_options()->DescriptorFile);
        return -1;
    }

    //originally pick_entry_guards(), now choose_random_entry().
    //pick_entry_guards(get_options());
...
build_exit_list()
{
    ...
    if (exit_node_list) {
        smartlist_free(exit_node_list);
    }

    exit_node_list = smartlist_create();

    SMARTLIST_FOREACH(current_consensus->routerstatus_list, routerstatus_t *, rs,
    if (rs->is_running && rs->is_valid && rs->is_exit) {
        if ((ri = router_get_by_digest(rs->identity_digest))) {
            r = compare_addr_to_addr_policy(0, port, ri->exit_policy);
            if (r == ADDR_POLICY_ACCEPTED || r == ADDR_POLICY_PROBABLY_ACCEPTED) {
                log_info(LD_OR,"Adding to exit list: %s, policy: %s\n", rs->nickname, rs->exitsummary);
                smartlist_add(exit_node_list, rs);
            }
        }
    });

    // Reset intermediate values used by exit selection code
    if (bandwidths) {
        tor_free(bandwidths);
        bandwidths = NULL;
    }
    if (ubandwidths) {
        tor_free(ubandwidths);
        ubandwidths = NULL;
    }
    if (exit_bits) {
        tor_free(exit_bits);
...
choose_circuit()
{
    const routerstatus_t *exit = NULL;
    const routerinfo_t *entry = NULL;
    const char *entry_identity_digest = NULL, *exit_identity_digest = NULL;

    tor_assert(exit_node_list);

    // This seems faster than routerinfo_sl_choose_by_bandwidth()
    exit = routerstatus_sl_choose_by_bandwidth(exit_node_list, WEIGHT_FOR_EXIT);
    exit_identity_digest = exit->identity_digest;

    do {
        entry = choose_random_entry(NULL);
        entry_identity_digest = entry->cache_info.identity_digest;
    } while (tor_memeq(exit_identity_digest, entry_identity_digest, DIGEST_LEN));

    printf("c %s;%s\n", router_describe(entry), routerstatus_describe(exit));
}
