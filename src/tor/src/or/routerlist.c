...
uint32_t *ubandwidths = NULL;
double weighted_bw = 0;
bitarray_t *exit_bits = NULL;
bitarray_t *guard_bits = NULL;
double exit_weight = 0;
double guard_weight = 0;
uint64_t total_bw = 0;
...
router_reload_router_list_impl(desc_store_t *store)
{
  ...
  store->journal_len = store->store_len = 0;
    ...
    // Kevin
    if (get_options()->Simulate && get_options()->DescriptorFile) {
        n = strlen(get_options()->DescriptorFile);
        fname = malloc(n + 1);
        memcpy(fname, get_options()->DescriptorFile, n);
        fname[n] = '\0';
    } else {
  fname = get_datadir_fname(store->fname_base);
    }
...
routers_have_same_or_addr(const routerinfo_t *r1, const routerinfo_t *r2)
{
    ...
    // Kevin
    if (get_options()->Simulate && r1->evil && r2->evil)
        return 0;
...
//   double *bandwidths;
  ...
  if (rule == WEIGHT_FOR_GUARD) {
    ...
    Wdb = networkstatus_get_bw_weight(NULL, "Wdb", -1);
        ...
        if (get_options()->Simulate) {
            log_info(LD_OR, "Simulating guard selection with weights \
                 Wg=%f Wm=%f We=%f Wd=%f Wgb=%f Wmb=%f Web=%f Wdb=%f",
                     Wg, Wm, We, Wd, Wgb, Wmb, Web, Wdb);
	    //printf("Simulating guard selection with weights Wg=%f Wm=%f We=%f Wd=%f Wgb=%f Wmb=%f Web=%f Wdb=%f\n", Wg, Wm, We, Wd, Wgb, Wmb, Web, Wdb);
        }
  ...
  bandwidths = tor_malloc_zero(sizeof(double)*smartlist_len(sl));
    ...
    log_info(LD_OR, "Setting up path selection");
  ...
  for (i = 0; i < (unsigned)smartlist_len(sl); ++i) {
    ...
    if (statuses) {
      ...
      routerinfo_t *router = smartlist_get(sl, i);
	    ...
	    //printf("Router %s ",router->nickname);
      ...
      if (rs && rs->has_bandwidth) {
		...
		//printf("Router %s, this_bw: %d ", router->nickname, this_bw);
		//
    ...
    if (is_guard && is_exit) {
      ...
      //printf("is a guard and exit and has weight=%f ",weight);
    } else if (is_guard) {
      weight = (is_dir ? Wgb*Wg : Wg);
      //printf("is a guard and has weight=%f ",weight);
    } else if (is_exit) {
    ...
    //printf("weighted_BW %f and cumulative weighted BW %f\n", bandwidths[i], weighted_bw);
  }
...
pick:
  ...
  sl_last_total_weighted_bw = weighted_bw;
    //printf("The total bandwidth that choose_by_bandwidth_weights found is: %lu \n",sl_last_total_weighted_bw);
    // Kevin
    log_info(LD_CIRC, "Choosing node for rule %s based on weights "
                              ...
                              int statuses)
{
  ...
  int is_exit;
  int is_guard;
    uint64_t total_nonexit_bw = 0, total_exit_bw = 0; /*, total_bw = 0;*/ // Kevin
   ...
   * and whether it was an exit (-x)&2 or guard (-x)&4.  Yes, it's a hack. */
        ubandwidths = tor_malloc(sizeof(int32_t)*smartlist_len(sl));
  exit_bits = bitarray_init_zero(smartlist_len(sl));
  guard_bits = bitarray_init_zero(smartlist_len(sl));
        exit_weight = guard_weight = total_bw = 0;
    }

    // Kevin
    log_info(LD_OR, "Setting up path selection");
    log_info(LD_OR, "router status list len: %d", (unsigned)smartlist_len(sl));
  ...
  for (i = 0; i < (unsigned)smartlist_len(sl); ++i) {
    ...
    if (statuses) {
      ...
      if (rs && rs->has_bandwidth) {
        this_bw = kb_to_bytes(rs->bandwidth);
                // Kevin
                //log_notice(LD_OR, "Summing over router %s bw %d guard %d exit %d", rs->nickname, this_bw, is_guard, is_exit);
  ...
  if (n_unknown) {
    ...
    for (i=0; i<(unsigned)smartlist_len(sl); ++i) {
            int32_t bw = ubandwidths[i];
      if (bw>=0)
        continue;
      is_exit = ((-bw)&2);
      is_guard = ((-bw)&4);
            ubandwidths[i] = ((-bw)&1) ? avg_fast : avg_slow;
      if (is_exit)
                total_exit_bw += ubandwidths[i];
      else
                total_nonexit_bw += ubandwidths[i];
      if (is_guard)
                total_guard_bw += ubandwidths[i];
      else
                total_nonguard_bw += ubandwidths[i];
    }
  }
  ...
  /* Figure out how to weight exits and guards */
  {
    ...
    for (i=0; i < (unsigned)smartlist_len(sl); i++) {
      ...
      if (is_exit && is_guard)
                bw = ((uint64_t)(ubandwidths[i] * exit_weight * guard_weight));
      else if (is_guard)
                bw = ((uint64_t)(ubandwidths[i] * guard_weight));
      else if (is_exit)
                bw = ((uint64_t)(ubandwidths[i] * exit_weight));
      else
                bw = ubandwidths[i];
      total_bw += bw;
...
pick:
  ...
  sl_last_total_weighted_bw = total_bw;
    ...
    // Kevin
    log_info(LD_CIRC, "Total weighted bw = "U64_FORMAT
            ", exit bw = "U64_FORMAT
  ...
  for (i=0; i < (unsigned)smartlist_len(sl); i++) {
    ...
    if (is_exit && is_guard)
            tmp += ((uint64_t)(ubandwidths[i] * exit_weight * guard_weight));
    else if (is_guard)
            tmp += ((uint64_t)(ubandwidths[i] * guard_weight));
    else if (is_exit)
            tmp += ((uint64_t)(ubandwidths[i] * exit_weight));
    else
            tmp += ubandwidths[i];
                          ...
                          router_crn_flags_t flags)
{
  ...
  const int weight_for_exit = (flags & CRN_WEIGHT_AS_EXIT) != 0;
    ...
    // Kevin
    log_info(LD_OR, "In router_choose_random_node(): Guard flag? %d", need_guard);
    // printf("In router_choose_random_node(): Guard flag? %d",need_guard);
  ...
  bandwidth_weight_rule_t rule;
    ...
    //printf("weight_for_exit? %d\n",weight_for_exit);
  tor_assert(!(weight_for_exit && need_guard));
  rule = weight_for_exit ? WEIGHT_FOR_EXIT :
    (need_guard ? WEIGHT_FOR_GUARD : WEIGHT_FOR_MID);
    //printf("rule for selection is: %d\n",rule);
  ...
  //	{
	  //	printf("Router %s has %llu bandwidth \n",r->nickname, r->bandwidthrate);
  //	count++;
  //	});	
	//printf("Guard_router_list has %d guards in it that are usable \n", count);
...
routerlist_remove(routerlist_t *rl, routerinfo_t *ri, int make_old, time_t now)
{
  ...
  tor_assert(0 <= idx && idx < smartlist_len(rl->routers));
    ...
    // Kevin
    log_info(LD_OR, "%s %d %s",
             ri->nickname, idx, ((routerinfo_t *) smartlist_get(rl->routers, idx))->nickname);
  ...
  if (have_enough_v2 && consensus) {
    ...
    for (i = 0; i < smartlist_len(routerlist->routers); ++i) {
      router = smartlist_get(routerlist->routers, i);
            // Kevin
            if (!get_options()->Simulate && router->cache_info.published_on <= cutoff &&
                                ...
                                const char *prepend_annotations)
{
  ...
  SMARTLIST_FOREACH_BEGIN(routers, routerinfo_t *, ri) {
    ...
    r = router_add_to_routerlist(ri, &msg, from_cache, !from_cache);
        ...
        // Kevin
//       log_info(LD_OR, "router_add_to_routerlist() returned %d", r);
  ...
  smartlist_free(changed);
    ...
    // Kevin
    //printf("router_load_routers_from_string(): Loaded %d routers\n", smartlist_len(routerlist->routers));
...
// Kevin
static int
_compare_routerstatus_by_id_digest(const void **a, const void **b)
{
    routerstatus_t *first = *(routerstatus_t **)a, *second = *(routerstatus_t **)b;
    return fast_memcmp(first->identity_digest,
...
// Kevin
void routerstatus_sort_by_identity(smartlist_t *routers)
{
    smartlist_sort(routers, _compare_routerstatus_by_id_digest);
}
