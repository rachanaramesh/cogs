...
// Kevin
routerinfo_t *add_an_entry_guard(routerinfo_t *chosen, int reset_status);
routerinfo_t *choose_good_entry_server(uint8_t purpose, cpath_build_state_t *state);
routerinfo_t *choose_good_exit_server(uint8_t purpose, routerlist_t *dir,
                                      int need_uptime, int need_capacity, int is_internal);
routerinfo_t *choose_good_middle_server(uint8_t purpose,
                                        cpath_build_state_t *state,
                                        crypt_path_t *head,
                                        int cur_len);
int onion_populate_cpath(origin_circuit_t *circ);
int onion_pick_cpath_exit(origin_circuit_t *circ, extend_info_t *exit);
void pick_entry_guards(or_options_t *options);
time_t get_consensus_time();
void log_entry_guards(int severity);
