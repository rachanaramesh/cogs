...
#define LOG_PROTOCOL_WARN (get_options()->ProtocolWarnings ? \
  ...
  int NumEntryGuards; /**< How many entry guards do we try to establish? */
    ...
    // Kevin
    int Simulate; // Should we simulate path selection? (default: no)
    int Patience;
    char *DescriptorFile; // What cached descriptor file should we use?
    char *ConsensusFile;  // What cached consensus file should we use?
    int NumPaths;   // How many paths should we simulate?
    int Time;     // The simulated time
    int NumMaliciousNodes; // Number of malicious nodes
    int MaliciousBandwidth; // Amount of bandwidth (in KiB/s) per malicious node
    int NumClients; // Number of simulated clients to run
    int GuardRotateMinDays; // Minimum time until we choose new guards (in days)
    int GuardRotateMaxDays; // Maximum time until we choose new guards (in days)
    int StartConsensus; //Starting Consensus
    int StopConsensus; //Ending Consensus
    char *ConsensusDir; // Where are the consensuses stored?
    char *DescriptorDir; //Where are the descriptors stored?
    int MaliciousGuard;  // Are our malicious nodes exits?
    int MaliciousExit;   // Are our malicious nodes guards?
    int NoChurn;		//Turn Churn off?
    int MaliciousIntroDate;       // Date Malicious guards appear
...
// Kevin: moved from circuitbuild.c
...
void
networkstatus_compute_bw_weights_v9(smartlist_t *chunks, int64_t G,
                                     int64_t M, int64_t E, int64_t D,
                                     int64_t T, int64_t weight_scale);
