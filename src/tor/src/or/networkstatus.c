...
// Kevin
networkstatus_t *current_consensus = NULL;
...
router_reload_consensus_networkstatus(void)
{
  ...
  for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
    ...
    if (flav == FLAV_NS) {
            // Kevin
            if (get_options()->Simulate && get_options()->ConsensusFile) {
                n = strlen(get_options()->ConsensusFile);
                filename = tor_malloc(n + 1);
                memcpy(filename, get_options()->ConsensusFile, n);
                filename[n] = '\0';
            } else {
                                    ...
                                    unsigned flags)
{
  ...
  if (flav != USABLE_CONSENSUS_FLAVOR &&
    ...
    // Kevin
    if (!get_options()->Simulate && (from_cache && !accept_obsolete) &&
  ...
  /* Make sure it's signed enough. */
    // Kevin
    if (!get_options()->Simulate && (r=networkstatus_check_consensus_signature(c, 1))<0) {
    if (r == -1) {
